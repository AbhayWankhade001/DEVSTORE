...
const BlogHero = () => {
  ...
  const fetchingBD = async () => {
    try {
      ...
      const updatedBlogItems = await Promise.all(
        data.map(async (item) => {
          const featuredMediaUrl = await fetchFeaturedMedia(item.id);
          return { ...item, featuredMediaUrl };
        })
      ...
      <div className="">
        <section className="relative bg-gray-50 pt-36">
          <div className="relative z-10 px-4 py-12 sm:py-16 sm:px-6 lg:px-8 lg:max-w-7xl lg:mx-auto lg:py-20 xl:py-28 lg:grid lg:grid-cols-2">
            ...
            <div ref={ref} className="pt-10 lg:-mb-10 lg:relative lg:inset-y-0 lg:inset-x-0 lg:pl-8">
              <div className="lg:w-full lg:max-w-none">
                {inView && (
                  <Slider ref={sliderRef} {...settings}>
                    {blogItemHero.map((blogItem) => (
                      <div key={blogItem.id} className="flex justify-center">
                        <div className="bg-white border border-gray-200 rounded-lg shadow-md css08">
                          {blogItem.featuredMediaUrl && (
                            <img
                              className="rounded-t-lg"
                              src={blogItem.featuredMediaUrl}
                              alt={blogItem.title.rendered}
                            />
                          )}
                          <div className="p-6">
                            <h2 className="mb-2 text-2xl font-bold text-gray-900">{blogItem.title.rendered}</h2>
                            <p className="text-gray-700">{stripHtmlAndLimitTextHero(blogItem.content.rendered)}</p>
                          </div>
                        </div>
                      </div>
                    ))}
